				     ...
				     struct ipu_bridge *bridge)
{
	...
	for_each_acpi_dev_match(adev, cfg->hid, NULL, -1) {
		...
		/* Remove any conflicting software node registered by the ACPI
		 * mipi-disco code before we register our full node group.
		 */
		{
			const struct software_node *existing =
				software_node_find_by_name(NULL, sensor->name);
			if (existing)
				software_node_unregister(existing);
		}
