...
KSRC=$(find "$REPO_ROOT/build" -maxdepth 1 -name 'linux-source-*' -type d 2>/dev/null | head -1)
if [[ -z "$KSRC" ]]; then
    echo "ERROR: kernel source not found under $REPO_ROOT/build/" >&2
    echo "       Move it there first, e.g.:" >&2
    echo "         mv ~/kernel-build/linux-source-$KVER_BASE $REPO_ROOT/build/" >&2
    exit 1
fi
